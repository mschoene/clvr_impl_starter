...
class ImageDecoder(nn.Module):
    def __init__(self, input_channels, output_size):
        ...
        self.conv_layers = nn.Sequential(
            #nn.Linear(64, 64 * 4 * 4),  # Linear layer to map from latent space to 4x4 feature map
            nn.Linear(64, 32 * 4 * 4),  # Linear layer to map from latent space to 4x4 feature map
            nn.ReLU(),
            #nn.Unflatten(1, (64, 4, 4)),  # Reshape to 4x4 feature map with 64 channels
            nn.Unflatten(1, (32, 4, 4)),  # Reshape to 4x4 feature map with 64 channels
            ...
            #nn.ReLU(),
            #nn.ConvTranspose2d(8, 3, kernel_size=4, stride=2, padding=1),   # Output size: [-1, 3, 64, 64]
