...
class PPO:
    def __init__(self, 
        ...
        self.n_actors = n_actors # defines how much off policy data is kept in the buffer, only 1 actor is exchanged each iteration
    ...
    def train(self):
        ...
        for iteration in range(self.n_episodes):
            ...
            collect_n_trajectories(self.n_trajectories, self.replayBuffer, self.model, self.env, self.n_traj_steps, self.gamma, self.lambda_val)
            ###
            ...
            if (len(self.replayBuffer) == self.buffer_size): #fill buffer fully first and then run
                for i_epoch in range(self.n_epochs):   
                    ...
                    for i_batch, sample_batched in enumerate(dataloader):
                        ...
                        log_std_penalty_loss = 0. 
                        if self.do_std_penalty:
                            log_std_penalty_loss = 0.2 * (self.model.action_std).mean()
