...
def main(args):
    ...
    def do_epochs(EPOCHS=1000):
        ...
        for epoch_number in range(EPOCHS):
            ...
            if (avg_vloss < best_vloss*0.5 and avg_vloss<0.008) or epoch_number==EPOCHS-1 or (epoch_number%100==0 and epoch_number >0):
                best_vloss = avg_vloss
                model_path = 'models/repr_decoder_{}_model_epoch_{}_{}'.format(train_type, epoch_number, timestamp)
                torch.save(decoder.state_dict(), model_path)
                model_path = 'models/repr_encoder_{}_model_epoch_{}_{}'.format(train_type, epoch_number, timestamp)
                torch.save(encoder.state_dict(), model_path)                
                model_path = 'models/repr_lstm_{}_model_epoch_{}_{}'.format(train_type, epoch_number, timestamp)
                torch.save(encoder.state_dict(), model_path)
